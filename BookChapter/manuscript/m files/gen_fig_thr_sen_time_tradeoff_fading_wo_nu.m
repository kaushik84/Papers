...
if sim
     ...
     N_sen_sim = [500:1000:20000];                                              % N = Total of samples used for sensing = tau * f_s 
    ...
    %% Computation of the estimation 
     ...
     for i=1:length(N_sen_sim)
        ...
        g_p1_sim = random('gam',m_p2, g_p2_true/m_p2, 1, M);                    % Power gain for the channel g_p2 (including path loss)
       ...
       %% Threshold     
        test_points = linspace(min(P_rcvd_ST_H1_sen_sim), max(P_rcvd_ST_H1_sen_sim),...
            num_test_points);
       ...
       %% Probability of false alarm and probability of detection
        ...
        P_d_id_sim(i) = length(find(P_rcvd_ST_H1_sen_sim > epsilon_id_sim(i)))/M;

       %% Expected Rate
        C_0_sim = mean(log2(1 + g_s_sim * P_s / noise_power)); 
        C_1_sim = mean(log2(1 + g_s_sim * P_s ./ (P_p * g_p2_sim +...
            noise_power)));
       ...
       %% non ideal case -- includes estimation followed by sesning
           ...
           %% Outage constraint            
            if 1
              %% Threshold                  
                ...
                C_0_sim = mean(log2(1 + g_s_est/noise_power));
                C_1_sim = mean(log2(1 + g_s_est./P_rcvd_SR_H1_est_sim));
    	...
    	save('results_thr_sen_time_tradeoff_fading_wo_nu_snr_m00_sim.mat');
    end
...
if th
   %% Theoretical parameters
    N_est_th = 5000;                                  % N_est_th = number of samples used for estimation = tau * f_s 
    N_sen_th = [10,100:100:5000,5001,5100:100:20000];    % N = includes the time in samples, for the ideal case their is no
...
%    epsilon_oc_th = zeros(1,length(N_sen_th));        % OC threshold
    ...
    parfor i=1:length(N_sen_th)
        ...
        %% Ideal         
        %% Threshold  
         if 0       % 1) Determining threshold by considering the outage constraint on the P_d
            epsilon_id_th = (2/(m_p2 * N_sen_th(i))*(g_p2_true * gammaincinv(1 - mu, m_p2, 'upper') +...
            noise_power) * gammaincinv(P_d_d, N_sen_th(i)/2, 'upper')); 
            
            %% Probability of detection
            func_exp_P_d = @(t) gammainc(N_sen_th(i) * epsilon_id_th./...
                (2 *( t *g_p2_true + noise_power)), N_sen_th(i)/2, 'upper') .*...
        ...
        C_0_th = integral(func_C0, 0, 100);
        C_1_th = integral2(func_C1, 0, 100, 0, 100);
        ...
        if N_sen_th(i) > N_est_th
           ...
           %% Outage case
            if 1
              %% Threshold  
                test_points = linspace(0.1 * epsilon_id_th, 10 * epsilon_id_th, num_test_points);
                ...
                [C_0_th, C_1_th] = calc_capacities_fad(P_p, P_s, g_p2_true, g_s_true,...
              ...
              %% Probability of false alarm and expected probability of dectection
                ...
                R_oc_th(i) = (K - N_sen_th(i))/K * (P_H0 * (1 -  P_f_oc_th(i)) * C_0_th +...
    ...
    save('results_thr_sen_time_tradeoff_fading_wo_nu_snr_m00_th.mat');
    quit;
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
load('results_thr_sen_time_tradeoff_fading_wo_nu_snr_m00_th.mat');
Fontsize = 8;
...
if 1
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    load('results_thr_sen_time_tradeoff_fading_wo_nu_snr_m00_sim.mat');
    diff = 2;
...
hl = legend([h1 h2 h4 h3],'IM', 'EM, Thm. 1', '$\trs(\test,\ttsen)$', 'Simulated');
...
if 0
    ...
    plot(N_sen_th(indexOfInterest) * 1e-3, R_id_th(indexOfInterest), 'c', 'LineWidth',4);
    hold on,
    plot(N_sen_th(indexOfInterest) * 1e-3, R_ac_th(indexOfInterest),'LineStyle', '--', 'LineWidth', 1);
    hold on,
    plot(N_sen_th(indexOfInterest) * 1e-3, R_oc_th(indexOfInterest), 'LineStyle', '-.', 'LineWidth', 1);
    ...
    index = 4;
    hold on,
    h4 = plot(N_sen_sim(index) * 1e-3, R_id_sim(index), 'kx', 'LineWidth',1);
    hold on,
    plot(N_sen_sim(index) * 1e-3, R_ac_sim(index), 'kx', 'LineWidth',1, 'HandleVisibility','off');
    hold on,
    plot(N_sen_sim(index) * 1e-3, R_oc_sim(index), 'kx', 'LineWidth',1, 'HandleVisibility','off');
