...
if analysis
        ...
        snr_s = 10.^([0]/10);                                   % received snr for the channel g_s  
        snr_p2 = 10.^([-10 0 10]/10);                           % snr for the channel g_p2  
        ...
        save('results_CDF_C1_diff_SNR_p2_SNR_s_00.mat');
end
load('results_CDF_C1_diff_SNR_p2_SNR_s_00.mat')
...
k = 1; 
plot(reshape(C1_pts(1, k, :), 1, M), reshape(CDF_C1_ana(1, k, :),1, M), 'c-', 'Linewidth',5);
hold on,
plot(reshape(C1_pts(1, k, sim), 1, length(sim)), reshape(CDF_C1_sim(1, k, sim), 1, length(sim)),...
    '-', 'Linewidth', 2);
...
laprint(1, '../figures/fig_CDF_C1_diff_SNR_p2_SNR_s_00', 'options', 'factory', 'width', 8, 'scalefonts',...
