...
if 1
        ...
        snr_p2 = 10.^([-00 10]/10);                             % snr for the channel g_p2  
        ...
        plot(reshape(C1_pts(1, 1, sim), 1, length(sim)), reshape(CDF_C1_sim(1, 1, sim), 1, length(sim)),...
            'kd', 'Linewidth',2.5);
        hold on,
        plot(reshape(C1_pts(1, 1, :), 1, M), reshape(CDF_C1_ana(1, 1, :),1, M), 'k', 'Linewidth',2.5);
        
        hold on,
        plot(reshape(C1_pts(2, 1, sim), 1, length(sim)), reshape(CDF_C1_sim(2, 1, sim), 1, length(sim)),...
            'kd', 'Linewidth',2.5);
        hold on,
        plot(reshape(C1_pts(2, 1, :), 1, M), reshape(CDF_C1_ana(2, 1, :),1, M), 'k', 'Linewidth',2.5);
        
        hold on,
        plot(reshape(C1_pts(3, 1, sim), 1, length(sim)), reshape(CDF_C1_sim(3, 1, sim), 1, length(sim)),...
            'kd', 'Linewidth',2.5);
        hold on,
        plot(reshape(C1_pts(3, 1, :), 1, M), reshape(CDF_C1_ana(3, 1, :),1, M), 'k', 'Linewidth',2.5);
               
        hold on,
        plot(reshape(C1_pts(1, 2, sim), 1, length(sim)), reshape(CDF_C1_sim(1, 2, sim), 1, length(sim)),...
            'kd', 'Linewidth',2.5);
        hold on,
        plot(reshape(C1_pts(1, 2, :), 1, M), reshape(CDF_C1_ana(1, 2, :),1, M), 'k', 'Linewidth',2.5);
      
        
        hold on,
        plot(reshape(C1_pts(2, 2, sim), 1, length(sim)), reshape(CDF_C1_sim(2, 2, sim), 1, length(sim)),...
            'kd', 'Linewidth',2.5);
        hold on,
        plot(reshape(C1_pts(2, 2, :), 1, M), reshape(CDF_C1_ana(2, 2, :),1, M), 'k', 'Linewidth',2.5);
      
        
        hold on,
        plot(reshape(C1_pts(3, 2, sim), 1, length(sim)), reshape(CDF_C1_sim(3, 2, sim), 1, length(sim)),...
            'kd', 'Linewidth',2.5);
        hold on,
        plot(reshape(C1_pts(3, 2, :), 1, M), reshape(CDF_C1_ana(3, 2, :),1, M), 'k', 'Linewidth',2.5);
      
        Fontsize = 9;
        grid on;
        ylabel('$\C1$ [bits/sec/Hz]','FontSize',Fontsize+2);
        xlabel('$\text(CDF)$','FontSize',Fontsize+2);
        hl = legend('Simulated', 'Theoretical');
        set(hl, 'Location', 'NorthEast', 'FontSize', Fontsize);
        set(gca,'FontSize',Fontsize);
        laprint(1, '../figures/fig_CDF_C_1_diff_SNR_s_SNR_p', 'options', 'factory', 'width', 8, 'scalefonts',...
