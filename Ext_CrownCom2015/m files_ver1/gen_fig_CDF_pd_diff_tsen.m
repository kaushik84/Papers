...
if analysis
        ...
        tsen = [1000 5000 10000];                       % Sensing time  
        ...
        for k = 1:length(tsen)
          ...
          %% Estimation of rcvd_energy is supposed to be non-central chi squared distributed 
            ...
            pd = gammainc(tsen(k)/2, tsen(k)/2 * threshold ./ rcvd_energy);        
            ...
            %% Anayltical CDF
            CDF_pd_ana(k,:) = gammainc(test/2, test * tsen(k) * threshold./...
                (4 * rcvd_energy_bar * gammaincinv(pd_pts(k,:), tsen(k)/2, 'upper')), 'upper');
