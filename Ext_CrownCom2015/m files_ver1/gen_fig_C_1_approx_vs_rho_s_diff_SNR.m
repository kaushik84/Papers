...
rho_p = 0.5;                                    % Channel correlation coefficient ST-SR
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Simulation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if sim   
    ...
    rho_s_sim = 0.1:0.1:0.95;                               % Channel correlation coefficient PT-SR    
    ...
    for i = 1:length(snr_p)
        ...
        for j = 1:length(snr_s)
            ...
            for k = 1:length(rho_s_sim)
                disp(strcat('SIM:rho_s_sim(j) = ',num2str(rho_s_sim(k))));
                g_s_sim = zeros(1,M);                                   % power gain for the channel g_s 
                g_p_sim = zeros(1,M);                                   % power gain for the channel g_p    
                parfor l = 1:M
                    g_s_sim(l) =  sum( normrnd(rho_s_sim(k), sqrt(snr_s(j))...
                        * sqrt(1 - rho_s_sim(k)^2), 1 ,dof_s).^2);
                    g_p_sim(l) =  sum( normrnd(rho_p, sqrt(snr_p(i))...
                        * sqrt(1 - rho_p^2), 1 ,dof_p).^2);
                end
                Exp_C_1_sim(i,j,k) = mean(log2(ones(1,M) + g_s_sim./(ones(1,M) + g_p_sim)));  
            end
        end
    end
    save('results_C_1_approx_vs_rho_s_diff_SNR_sim.mat', 'snr_p','snr_s',...
        'Exp_C_1_sim', 'rho_s_sim', 'rho_p');
   ...
   %% Theoretical parameters
    ...
    for i = 1:length(snr_p)           
        ...
        for j = 1:length(snr_s)           
            ...
            for k = 1:length(rho_s_th)
                disp(strcat('TH:rho_s_th(k) = ',num2str(rho_s_th(k))));

                %% Gamma Parameters
                lambda_s = dof_s *  rho_s_th(k)^2/(snr_s(j) * (1 - rho_s_th(k)^2));   
                mean_ana_s = snr_s(j) * (1 - rho_s_th(k)^2) * (dof_s + lambda_s);    
                var_ana_s = snr_s(j)^2 * (1 - rho_s_th(k)^2)^2 * (2 * dof_s + 4 * lambda_s);

                b_gamma_s = var_ana_s/mean_ana_s;
                a_gamma_s = mean_ana_s/b_gamma_s;

                lambda_p = dof_p *  rho_p^2/(snr_p(i) * (1 - rho_p^2));   
                mean_ana_p = snr_p(i) * (1 - rho_p^2) * (dof_p + lambda_p);    
                var_ana_p = snr_p(i)^2 * (1 - rho_p^2)^2 * (2 * dof_p + 4 * lambda_p);
                ...
                if 1
                    f_exp_C_1_s = @(y,z) z .* (2.^z) .* log(2) * 1/(gamma(a_gamma_s) * b_gamma_s^a_gamma_s)...
                       * 1/(gamma(a_gamma_p2) * b_gamma_p2^a_gamma_p2) .* y .* ((2.^z - 1) .* y).^(a_gamma_s-1)...
                       ...
                       exp(-(y-1)/b_gamma_p2);
                    if rho_s_th(k) >= 0.96 
                        Exp_C_1_th(i,j,k) = integral2(f_exp_C_1_s, 1, 20, 0, 10);
                    else
    ...
    save('results_C_1_approx_vs_rho_s_diff_SNR_th.mat');
    %quit;
end
...
Fontsize = 9;
if 1
    load('results_C_1_approx_vs_rho_s_diff_SNR_th.mat');
    plot(rho_s_th, reshape(Exp_C_1_th(1,1,:), 1, length(rho_s_th)),'-', 'LineWidth', 1.5);
    %hold on, 
    %plot(rho_s_th, reshape(Exp_C_1_th(1,2,:), 1, length(rho_s_th)),'-', 'LineWidth', 1.5, 'HandleVisibility','off');
    hold on, 
    plot(rho_s_th, reshape(Exp_C_1_th(1,3,:), 1, length(rho_s_th)),'-', 'LineWidth', 1.5, 'HandleVisibility','off');
    hold on, 
    plot(rho_s_th, reshape(Exp_C_1_th(2,1,:), 1, length(rho_s_th)),'--', 'LineWidth', 1.5, 'HandleVisibility','off');
    %hold on, 
    %plot(rho_s_th, reshape(Exp_C_1_th(2,2,:), 1, length(rho_s_th)),'--', 'LineWidth', 1.5, 'HandleVisibility','off');
    hold on, 
    plot(rho_s_th, reshape(Exp_C_1_th(2,3,:), 1, length(rho_s_th)),'--', 'LineWidth', 1.5, 'HandleVisibility','off');
    hold on,
    plot(rho_s_th, reshape(Exp_C_1_th(3,1,:), 1, length(rho_s_th)),'-.', 'LineWidth', 1.5, 'HandleVisibility','off');
    %hold on,
    %plot(rho_s_th, reshape(Exp_C_1_th(3,2,:), 1, length(rho_s_th)),'-.', 'LineWidth', 1.5, 'HandleVisibility','off');
    hold on,
    plot(rho_s_th, reshape(Exp_C_1_th(3,3,:), 1, length(rho_s_th)),'-.', 'LineWidth', 1.5, 'HandleVisibility','off');    
end
if 1
    load('results_C_1_approx_vs_rho_s_diff_SNR_sim.mat');
    hold on,
    plot(rho_s_sim, reshape(Exp_C_1_sim(1,1,:), 1, length(rho_s_sim)), 'r*');    
    %hold on,
    %plot(rho_s_sim, reshape(Exp_C_1_sim(1,2,:), 1, length(rho_s_sim)), 'r*', 'HandleVisibility','off');    
    hold on,
    plot(rho_s_sim, reshape(Exp_C_1_sim(1,3,:), 1, length(rho_s_sim)), 'r*', 'HandleVisibility','off');
    hold on, 
    plot(rho_s_sim, reshape(Exp_C_1_sim(2,1,:), 1, length(rho_s_sim)), 'r*', 'HandleVisibility','off');    
    %hold on, 
    %plot(rho_s_sim, reshape(Exp_C_1_sim(2,2,:), 1, length(rho_s_sim)), 'r*', 'HandleVisibility','off');
    hold on, 
    plot(rho_s_sim, reshape(Exp_C_1_sim(2,3,:), 1, length(rho_s_sim)), 'r*', 'HandleVisibility','off');
    hold on,
    plot(rho_s_sim, reshape(Exp_C_1_sim(3,1,:), 1, length(rho_s_sim)), 'r*', 'HandleVisibility','off');
    %hold on,
    %plot(rho_s_sim, reshape(Exp_C_1_sim(3,2,:), 1, length(rho_s_sim)), 'r*', 'HandleVisibility','off');
    hold on,
    plot(rho_s_sim, reshape(Exp_C_1_sim(3,3,:), 1, length(rho_s_sim)), 'r*', 'HandleVisibility','off');    
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
axis([min(rho_s_th) max(rho_s_th)...
    min(reshape(Exp_C_1_th(3,1,:), 1, length(rho_s_th))) * 0.0....
    max(reshape(Exp_C_1_th(1,3,:), 1, length(rho_s_th))) * 1.05]);
ylabel('$\C1$','FontSize',Fontsize+2);
xlabel('$\rho_s$ = [ms]','FontSize',Fontsize+2);
hl = legend('Theoretical', 'Simulated');
set(hl, 'Location', 'NorthEast', 'FontSize', Fontsize);
set(gca,'FontSize',Fontsize);
laprint(1, '../figures/fig_C_1_approx_vs_rho_s_diff_SNR', 'options', 'factory', 'width', 8, 'scalefonts',...
