...
mu = 0.15;                                % Outage Probability on probability of detection
...
if th
   %% Theoretical parameters
        ...
        for j=1:length(N_est_th)
            ...
            parfor k=1:length(N_sen_th)
               ...
               %% Probability of false alarm and probability of dectection
                P_f_id_temp(k) = gammainc(N_sen_th(k)/2, N_sen_th(k) * epsilon_id_th/(2 * noise_power));        
                P_d_id_temp(k) =  P_d_d;
                ...
                if N_sen_th(k) > N_est_th(j)
                   ...
                   if Adjacent
                       ...
                       for l=1:num_test_points
                            func_exp_P_d = @(t) gammainc(N_sen_ni_th/2, N_sen_ni_th/2 * test_points(l) ./ t) *...
                       ...
                       %% Probability of false alarm and probability of dectection
                       P_f_ac_temp(k) = gammainc(N_sen_ni_th/2, N_sen_ni_th * epsilon_ac_th/...
                       (2 * noise_power));                               
                    ...
                    if Outage
                        ...
                        P_f_oc_temp(k) = gammainc(N_sen_ni_th/2, N_sen_ni_th * epsilon_oc_th/...
                            (2 * noise_power));     
    ...
    save('results_opt_thr_P_d_P_f_vs_est_time_diff_SNR_wo_nu_m10_mu15_oc_th.mat');
    quit;
end
...
if 0
    ...
    %   time
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    load('results_opt_thr_P_d_P_f_vs_est_time_diff_SNR_wo_nu_m10_mu05_oc_th.mat');
    ...
    figure(1);
    plot(N_est_th(pts) * 1e-3, P_d_id_th(1,pts),'c-', 'LineWidth', 5);
    hold on, 
...
if 0
    ...
    semilogy(N_est_th(pts1) * 1e-3, P_f_id_th(1,pts1),'c-', 'LineWidth', 5);
    hold on,
    ...
    semilogy(N_est_th(pts1) * 1e-3, P_f_oc_th(1,pts1),'--', 'LineWidth', 2);    
        ...
        min(P_f_id_th(1,pts1)) * 0.5 max(P_f_oc_th(1,pts1))]);
...
if 1
    ...
    plot(N_est_th(scale) * 1e-3, R_id_opt_th(1,(scale)),'c-','LineWidth', 5);
    hold on,
    plot(N_est_th(scale) * 1e-3, R_ac_opt_th(1,(scale)),'-', 'LineWidth', 2);
    hold on,
    plot(N_est_th(scale) * 1e-3, R_oc_opt_th(1,(scale)),'--', 'LineWidth', 2);
    ...
    if 1
        ...
        hold on,
        plot(N_est_th(scale) * 1e-3, R_oc_opt_th(1,(scale)),'--', 'LineWidth', 2, 'HandleVisibility','off');
