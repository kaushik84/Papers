   ...
   %% Theoretical parameters
    ...
    for i = 1:length(P_p) 
        ...
        parfor j=1:length(N_sen_th)
            ...
            P_f_id_th(i,j) = gammainc(N_sen_th(j)/2, N_sen_th(j) * epsilon_id_th/(2 * noise_power));
            P_d_id_th(i,j) =  P_d_d;
            ...
            if N_sen_th(j) > N_est_th
               ...
               if 1
                   ...
                   for k=1:num_test_points
                        func_exp_P_d = @(t) gammainc(N_sen_ni_th/2, N_sen_ni_th/2 * test_points(k) ./ t) *...
                   ...
                   %% Probability of false alarm and probability of dectection   
                   P_f_ac_th(i,j) = gammainc(N_sen_ni_th/2, N_sen_ni_th * epsilon_ac_th/...
                       (2 * noise_power));  
                ...
                if 1
                    ...
                    P_f_oc_th(i,j) = gammainc(N_sen_ni_th/2, N_sen_ni_th * epsilon_oc_th/...
                        (2 * noise_power));   
...
figure(1);
Fontsize = 9;
plot(10 *log10(P_p), R_id_opt_th, 'c-', 'LineWidth',5);
hold on,
plot(10 *log10(P_p), R_ac_opt_th, 'b-', 'LineWidth', 2);
hold on,
plot(10 *log10(P_p), R_oc_opt_th, 'b-.', 'LineWidth', 2);
...
set(hl, 'position',[0.615 0.12 0.28 0.21]);
...
figure(2);
Fontsize = 9;
plot(10 *log10(P_p), P_f_id_opt_th, 'b', 'LineWidth', 2);
hold on,
plot(10 *log10(P_p), P_f_ac_opt_th, 'LineStyle', '--', 'LineWidth', 2);
hold on,
plot(10 *log10(P_p), P_f_oc_opt_th, 'LineStyle', '-.', 'LineWidth', 2);
