...
if 0
    ...
    alpha_true_p = [140:-0.005:90];                     % True Path loss between ST and PR  to achieve P_p = IT  
...
if 1
    ...
    P_reg_max = [-10 0];                              % Maximum Controlled Power at the ST
    ...
    if th
             ...
             for i=1:length(P_reg_max)
                 ...
                 for j=1:length(epsilon)
                    ...
                    for k=1:length(N_th)       
                           ...
                           temp1 = 1 - gammainc((10^(I_T/10) * 10^(P_tran/10)...
                               /10^(P_reg_max(i)/10) + 10^(noise_power/10))./b, a);
                         ...
                         if 0
                             P_reg = min(10000, 10^(I_T/10) * 10^(P_tran/10) ./...
                                    (b .* gammaincinv((1 - epsilon(j)),a) - 10^(noise_power/10)));  
                             temp2 = 1 - gammainc((10^(I_T/10) * 10^(P_tran/10)...
                                   ./P_reg + 10^(noise_power/10))./b, a); 
                             [value index2] = min(abs(temp2 - epsilon(j)));
                             SNR_I3(i,j,k) = 10 * log10(snr(index2));
                         end
                    end 
                 end
             end
             save('results_N_vs_SNR_diff_pout_diff_maxContPow_th');
         end
     end
     load('results_N_vs_SNR_diff_pout_diff_maxContPow_th');
     Fontsize = 8.5;
     figure(1);
     ...
     hold on, 
     h3 = plot(reshape(SNR_I2(1,2,:),1, length(N_th)), N_th * 1e-3,  'k--', 'LineWidth', 1); 
     %hold on, 
     ...
     %set(hl, 'position',[0.725 0.12 0.15 0.31]);
     set(hl, 'position',[0.14 0.71 0.285 0.2]);
     set(gca,'FontSize',Fontsize);
