...
if sim
    ...
    PO_P_p_sim = zeros(1,length(N_sim));      % Outage probability power at PR  
...
if th
   %% Theoretical parameters
    ...
    PO_P_p_th = zeros(1,length(N_th));          % Outage probability power at PR  
    nc_th = 1;

    for i=1:length(N_th)       
        disp(strcat('N = ',num2str(i)));             
     
        nc_th = (10^(noise_power/10)/10^(-alpha_true_p/10) + 10^(P_tran/10));
        
       %% Determining the performance meterics --> Exp_R, CP_power, CP_rate
       %% Expected values            
        Exp_P_p_th = 10^(I_T/10);
       
        PO_P_p_th(i) = marcumq(sqrt(10^(-alpha_true_p/10) * 10^(P_tran/10) * ...
            N_th(i)/(10^(noise_power/10))),...
            sqrt(10^(I_T/10) * nc_th * N_th(i) * 10^(-alpha_true_p/10)/...
            (10^(noise_power/10) * (1 + mu_P) * Exp_P_p_th)), ceil(N_th(i)/2 - 1));
