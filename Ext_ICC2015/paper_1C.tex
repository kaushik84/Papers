...
\item Based on the proposed model, we capture the effect of imperfect channel knowledge by employing an outage constraint on the received power at the PR that restrains the interference encountered by a primary system. Subsequently, we investigate a tradeoff between the estimation time and the achievable secondary throughput. %expression of estimation-throughput tradeoff that analyzes the performance of the US. Using this tradeoff, we determine the maximum achievable throughput for the secondary system. %As a result of the analysis, we propose a power control scheme at the ST subject to outage probability constraint at the PR. 
%\item 
...
\figurename~\ref{fig:ETT} analyzes performance of US in terms of estimation-throughput tradeoff, cf. Theorem \ref{th:th1}, corresponding to the Ideal Model (IM) and the Estimation Model (EM). %Clearly, by relaxing the $\opc$, an improvement in performance it terms of $\trs$ is observed. 
...
\draw (0.225,0.65) arc(-160:160:0.01 and 0.03);
\node[draw, fill=gray!10, font=\scriptsize] (text1) at (0.225,0.73) {$\pc = \SI{00}{dBm}$};
\draw (0.225,0.375) arc(-160:160:0.01 and 0.03);
\node[draw, fill=gray!10, font=\scriptsize] (text2) at (0.225,0.455) {$\pc = \SI{-10}{dBm}$};
\draw (0.63,0.66) arc(-150:150:0.016 and 0.048);
\draw[black, <-] (text1.east) -- (0.64,0.73);
\draw (0.63,0.58) arc(-160:160:0.01 and 0.03);
\draw[black, <-] (text2.east) -- (0.635,0.565);
...
\end{tikzpicture}
\vspace{-4mm}
\caption{Optimum throughput $(\rs(\ttau))$ versus the ratio of received control-based power to noise $(\gamma)$ with $\opc \in \{0.01, 0.1\}$ and $\pc \in \{-10, 0\}$ \SI{}{dBm}.}
\label{fig:optT_snr}
\vspace{-10mm}
\end{figure}
...
%\begin{figure}[!t]
%% Add psfrag entries
%\input{figures/fig_opc_vs_SNR_diff_N_diff_maxContPow_th}
%\centering
%\includegraphics[width= \figscale]{figures/fig_opc_vs_SNR_diff_N_diff_maxContPow_th}
%\caption{Operation regime for the US depicted in terms of the outage probability constraint and the received SNR.}
%\label{fig:_snr}
%\vspace{-5mm}
%\end{figure}
...
%\begin{figure*}[!t]
%\centering
%\subfloat[$\rs(\ttau)$ versus $\gamma$]{
%\input{figures/fig_opt_thr_vs_SNR_AWGN.tex}
%\includegraphics[width= 0.45 \columnwidth]{figures/fig_opt_thr_vs_SNR_AWGN}
%\quad %\label{fig:OptT_snr}
%}
%\hfil
%\subfloat[$\ttau$ versus $\gamma$]{
%\input{figures/fig_opt_est_time_vs_SNR_AWGN.tex}
%\includegraphics[width= 0.45 \columnwidth]{figures/fig_opt_est_time_vs_SNR_AWGN}
%\caption{(a) Optimum throughput and (b) Optimum estimation time vs SNR received at the ST the short-term performance characterization of the US.}
%%\label{fig:optEst_snr}
%\quad
%}
%\end{figure*}
...
%\caption{Estimation-throughput tradeoff for the long-term characterization.}
%\label{fig:optT_snr}
%\end{figure}
...
%\caption{Optimum throughput vs SNR received at the ST for the long-term characterization.}
%\label{fig:optT_snr}
%\end{figure}
...
%\subfloat[$\rs(\ttau)$ versus $\gamma$]{
%\input{figures/fig_opt_thr_vs_SNR_fading.tex}
%\includegraphics[width= 0.45 \columnwidth]{figures/fig_opt_thr_vs_SNR_fading}
%\quad %\label{fig:OptT_snr}
%}
%\hfil
%\subfloat[$\ttau$ versus $\gamma$]{
%\input{figures/fig_opt_est_time_vs_SNR_fading.tex}
%\includegraphics[width= 0.45 \columnwidth]{figures/fig_opt_est_time_vs_SNR_fading}
%\caption{(a) Optimum throughput and (b) Optimum estimation time vs SNR received at the ST the short-term performance characterization of the US.}
%%\label{fig:optEst_snr}
%\quad
%}
