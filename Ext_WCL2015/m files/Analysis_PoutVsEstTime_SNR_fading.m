...
if 0
    ...
    alpha_true_p = [180:-0.05:90];                     % True Path loss between ST and PR  to achieve P_p = IT  
    ...
    P_reg_max = [1];                           % Maximum Controlled Power at the ST
...
if 1
    ...
    P_reg_max = [01];                                  % Maximum Controlled Power at the ST
    epsilon = [0.01 0.1];                              
    m_p = [0.5 1 2 5 100];                                 % Number of realizations of channel for the link ST-PR

    %N_th = [1:2:200,200:40:2000,10000];                % N = Total of samples used for estimation = tau * f_s         
    ...
    SNR_I3 = zeros(length(P_reg_max), length(epsilon),....
        length(N_th));                                % With Regulated Power and transmit power constraint  
    ...
    if th
        ...
        %% Issue 2 Estimation model
         if 1    
             for i=1:length(epsilon)
                 disp(strcat('epsilon = ',num2str(epsilon(i))));   
                 for j=1:length(m_p)
                    disp(strcat('m_p = ',num2str(m_p(j))));   
                    for k=1:length(N_th)                 
                        disp(strcat('N_th = ',num2str(N_th(k))));  
                        parfor l=1:length(snr)
                           warning off;     
                           func_prcvd = @(t) gammainc((10^(I_T/10) * 10^(P_tran/10)/P_reg_max...
                            + 10^(noise_power/10))./(10^(noise_power/10)/N_th(k)...
                            * (2 + 4 * t * snr(l))./(1 + t * snr(l))),...
                            N_th(k) * (1 + t * snr(l)).^2./(2 + 4 * t * snr(l)), 'upper')*...
                            1/gamma(m_p(j)) .* t.^(m_p(j) - 1) * m_p(j)^(m_p(j)) .* exp(-m_p(j) * t);

                            %% Determine the SNR that achieves the outage probability
                            temp1(l) = integral(func_prcvd, 0, 100);
                        end                          
                           
                        [value index1] = min(abs(temp1 - epsilon(i) * ones(1, length(snr))));
                        SNR_I2(i,j,k) = 10 * log10(snr(index1));
                        disp(strcat('SNR_I2 = ',num2str(SNR_I2(i,j,k))));  
                        disp(strcat('epsilon = ',num2str(value))); 
                    end
                 end             
             end
             save('results_N_vs_SNR_diff_pout_diff_m_th'); 
         end
         quit;
     end
     load('results_N_vs_SNR_diff_pout_diff_m_th');
     Fontsize = 8;
     figure(1);
     ...
     if 0
         ...
         hold on, 
         plot(reshape(SNR_I2(1,2,:),1, length(N_th)), N_th * 1e-3,  'k-', 'LineWidth', 1); 
         hold on, 
         plot(reshape(SNR_I2(1,3,:),1, length(N_th)), N_th * 1e-3,  'k-', 'LineWidth', 1); 
         hold on, 
         plot(reshape(SNR_I2(1,4,:),1, length(N_th)), N_th * 1e-3,  'k-', 'LineWidth', 1); 
         hold on, 
         plot(reshape(SNR_I2(1,5,:),1, length(N_th)), N_th * 1e-3,  'k-', 'LineWidth', 1); 
     end
     
     h3 = plot(reshape(SNR_I2(2,1,:),1, length(N_th)), N_th * 1e-3,  'k-', 'LineWidth', 1); 
     hold on, 
     ...
     hold on, 
     plot(reshape(SNR_I2(2,4,:),1, length(N_th)), N_th * 1e-3,  'k-', 'LineWidth', 1); 
     hold on, 
     plot(reshape(SNR_I2(2,5,:),1, length(N_th)), N_th * 1e-3,  'k-', 'LineWidth', 1); 
     ...
     %set(hl, 'position',[0.725 0.12 0.15 0.31]);
     %set(hl, 'position',[0.14 0.76 0.30 0.15]);
     set(gca,'FontSize',Fontsize);
     laprint(1, '../figures/fig_N_vs_SNR_diff_pout_diff_maxContPow_fading_th', 'options', 'factory', 'width', 8, 'scalefonts',...
        'on', 'factor',0.5, 'keepfontprops', 'on'); 
end
