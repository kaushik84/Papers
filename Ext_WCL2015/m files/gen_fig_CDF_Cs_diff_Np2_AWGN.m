...
if analysis
        ...
        alpha_p2_true = 10.^(-[110]/10);                        % Secondary Interference Channel 
        ...
        save('results_CDF_Cs_diff_Np2_AWGN.mat');
end
load('results_CDF_Cs_diff_Np2_AWGN.mat')
...
if diss 
    ...
    k = 3;
    hold on,
    lp1 = plot(reshape(Cs_pts(k, 1, :), 1, M), reshape(CDF_Cs_ana(k, 1, :),1, M), 'c-', 'Linewidth',1);
    hold on,
    plot(reshape(Cs_pts(k, 1, sim), 1, length(sim)), reshape(CDF_Cs_sim(k, 1, sim), 1, length(sim)),...
        '+', 'Linewidth', 1);
    ...
    laprint(1, '../figures/fig_CDF_Cs_diff_Np2_AWGN', 'options', 'factory', 'width', 8, 'scalefonts',...
    ...
    k = 1; 
    plot(reshape(Cs_pts(k, 1, :), 1, M), reshape(CDF_Cs_ana(k, 1, :),1, M), 'b-', 'Linewidth',1);
    hold on,
    plot(reshape(Cs_pts(k, 1, sim), 1, length(sim)), reshape(CDF_Cs_sim(k, 1, sim), 1, length(sim)),...
        'x', 'Linewidth', 1);
    ...
    k = 3;
    hold on,
    lp1 = plot(reshape(Cs_pts(k, 1, :), 1, M), reshape(CDF_Cs_ana(k, 1, :),1, M), 'b-', 'Linewidth',1);
    hold on,
    plot(reshape(Cs_pts(k, 1, sim), 1, length(sim)), reshape(CDF_Cs_sim(k, 1, sim), 1, length(sim)),...
        '+', 'Linewidth', 1);
    ...
    xlabel('$\eca$ [bits/sec/Hz]','FontSize',Fontsize);
    ylabel('CDF','FontSize',Fontsize);
    hl = legend([lp1, lp2, lp3], '$\tau = 0.1$ ms', '$\tau = 1$ ms',...
        '$\tau = 10$ ms');
    set(hl, 'position',[0.62 0.12 0.28 0.2]);
    set(gca,'FontSize',Fontsize);
    laprint(1, '../figures/fig_CDF_Cs_diff_Np2_AWGN', 'options', 'factory', 'width', 8, 'scalefonts',...
        'on', 'factor',0.5, 'keepfontprops', 'on');  
end
