...
if analysis
        ...
        N_p2 = [100 1000 10000];                                % Number of samples used for estimation
        noise_power = 10^(-100/10);                             % Noise power at SR 
        N_s = 10;                                               % Number of Pilot Symobols 
        m_p = 1;                                                % Nakagam-m fading paramter    
        m_s = 1;        
        ...
        g_p2_est = ones(1, M) .* random('gam',m_p, 1/m_p, 1, M);
        g_s_est = ones(1, M) .* random('gam', m_s, 1/m_s, 1, M); 
        for k = 1:length(N_p2)
              disp(strcat('N_p2 = ',num2str(N_p2(k))));
	      for l = 1:length(alpha_p2_true)
              %% Simulation
               ...
               %% CDF of Capacity C_1 = log2(1 + SNR_s)         
                 ...
                 parfor i = 1:length(Cs_pts(k,l,:))
                     warning off;
		     func_den_Cs = @(t, v, u) log(2) .* exp(log(2.^t)  + (N_s*(1+u*g_s/np).^2./(2+u*4*g_s/np) - 1)...
                        .* log(2.^t - 1) - (N_s*(1+u*g_s/np).^2./(2+u*4*g_s/np) + N_p2(k)*(1+v*snr_p2(l)).^2./(2+v*4*snr_p2(l))) .*...
                        log(1./(np/N_p2(k)*(2+v*4*snr_p2(l))./(1+v*snr_p2(l))) + (2.^t - 1)./(np*Preg/N_s * (2+u*4*g_s/np)./(1+u*g_s/np)))...
                        + gammaln(N_p2(k)*(1+v*snr_p2(l)).^2./(2+v*4*snr_p2(l)) + N_s*(1+u*g_s/np).^2./(2+u*4*g_s/np))...
                        - gammaln(N_p2(k)*(1+v*snr_p2(l)).^2./(2+v*4*snr_p2(l))) - gammaln(N_s*(1+u*g_s/np).^2./(2+u*4*g_s/np))...
                        - N_p2(k)*(1+v*snr_p2(l)).^2./(2+v*4*snr_p2(l)) .* log(np/N_p2(k)*(2+v*4*snr_p2(l))./(1+v*snr_p2(l)))...
        ...
        save('results_CDF_Cs_diff_SNR_p2_fading.mat');
        quit;
end
load('results_CDF_Cs_diff_SNR_p2_fading.mat')
...
laprint(1, '../figures/fig_CDF_Cs_diff_SNR_p2_fading', 'options', 'factory', 'width', 8, 'scalefonts',...
