...
%% Ideal Model
if 1 
    P_reg_id = 10^(I_T/10) / 10^(-alpha_true_p1/10);  
end
...
hold on,
semilogx(downsampled(2:end) * 1e-3, 10*log10(P_reg_id) * ones(1, length((downsampled(2:end)))), 'cd', 'LineWidth', 1);
lp1 = plot(0,0,'-c', 'Marker','d','LineWidth', 1,'visible','off');
...
grid on;
axis([0 max(N_th) * 1e-3 Y_min  Y_max * 0.9]);
ylabel('$\preg$ [dBm]','FontSize',Fontsize);
xlabel('$\tau$ [ms]','FontSize',Fontsize);
hl = legend([lp1 h2],'IM', 'EM', 'Simulated');
%set(hl, 'position',[0.725 0.12 0.15 0.31]);
set(hl, 'position',[0.7 0.12 0.2 0.15]);
set(gca,'FontSize',Fontsize);
laprint(2, '../figures/fig_Preg_est_time_AWGN', 'options', 'factory', 'width', 8, 'scalefonts',...
    'on', 'factor',0.5, 'keepfontprops', 'on');
