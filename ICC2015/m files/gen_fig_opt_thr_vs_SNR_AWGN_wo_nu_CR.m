...
%   Parameters
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
P_tran = -00;                             % SNR at ST can be controlled by changing the 
...
alpha_true_p = 100 + [15:-0.2:-15];        % True Path loss between ST and PR  to achieve P_p = IT  
alpha_true_s = 080;                       % True Path loss between ST and SR  to achieve P_p = IT  
epsilon_P = [0.92 0.95 0.97];             % Probability constraint
...
if sim
    ...
    save('results_opt_thr_vs_snr_AWGN_wo_nu_sim.mat');
...
if th
    ...
    Exp_R_opt = zeros(length(epsilon_P),length(alpha_true_p));        % Expected power regulated by ST
    N_optimum = zeros(length(epsilon_P),length(alpha_true_p));        % Optimum Sensing Time ST

    for e = 1:length(epsilon_P)
        parfor s = 1:length(alpha_true_p)    

            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %   Finding optimum Rate by determing the optimum sensing time
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            nc_th = (10^(noise_power/10)/10^(-alpha_true_p(s)/10) + 10^(P_tran/10));
            disp(strcat('SNR = ',num2str(P_tran - noise_power - alpha_true_p(s)))); 
            ...
            for i = length(N_opt):-1:1           
                ...
                PC_P_p_opt = marcumq(sqrt(10^(-alpha_true_p(s)/10) * 10^(P_tran/10) * ...
                    N_opt(i)/(10^(noise_power/10))),...
                    sqrt(10^(I_T/10) * nc_th * N_opt(i) * 10^(-alpha_true_p(s)/10)/...
                    (10^(noise_power/10) * (1 + mu_P) * Exp_P_p_opt)), ceil(N_opt(i)/2 - 1)) -... 
                marcumq(sqrt(10^(-alpha_true_p(s)/10) * 10^(P_tran/10) * ...
                    N_opt(i)/(10^(noise_power/10))),...
                    sqrt(10^(I_T/10) * nc_th * N_opt(i) * 10^(-alpha_true_p(s)/10)/...
                    (10^(noise_power/10) * (1 - mu_P) * Exp_P_p_opt)), ceil(N_opt(i)/2 - 1));
            ...
            Exp_R_opt(e, s) = (K- N_optimum(e, s))/K * log2(1 + 10^(I_T/10)/(10^(P_tran/10) *  10^(-alpha_true_p(s)/10) +...
                    10^(noise_power/10)) * nc_th * 10^(-alpha_true_s/10)  / 10^(noise_power/10));
            disp(strcat('Exp_R_opt = ',num2str(Exp_R_opt(e, s))));
        end
    end
    save('results_opt_thr_vs_snr_AWGN_wo_nu_th_10_CR.mat');
    quit;
end
load('results_opt_thr_vs_snr_AWGN_wo_nu_th_10_CR.mat');
...
if 0 
    load('results_opt_thr_vs_snr_AWGN_wo_nu_sim.mat');
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
