...
epsilon_P = [0 0.92, 0.95, 0.97];           % Probability constraint
...
if sim
    ...
    save('results_opt_thr_vs_acc_AWGN_wo_nu_sim.mat');
...
if th
        ...
        parfor m = 1:length(mu_P_th)    
            ...
            for i = length(N_opt):-1:1           
                ...
                PC_P_p_opt = marcumq(sqrt(10^(-alpha_true_p/10) * 10^(P_tran/10) * ...
                    ...
                    (10^(noise_power/10) * (1 + mu_P_th(m)) * Exp_P_p_opt)), ceil(N_opt(i)/2 - 1)) -... 
    ...
    save('results_opt_thr_vs_acc_AWGN_wo_nu_th.mat');
    quit;
end
...
if 0 
    load('results_opt_thr_vs_acc_AWGN_wo_nu_sim.mat');
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
