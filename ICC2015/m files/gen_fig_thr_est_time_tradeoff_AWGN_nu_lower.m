...
P_tran = -00;                             % Power transmitted by PR, the SNR received at ST can be 
                                          % varried using this parameternoise_power = -100;                       % Actual noise power -100 dBm
...
if sim
    ...
    save('results_thr_est_time_tradeoff_AWGN_nu_l_snr_m00_sim.mat');
...
if th
   %% Theoretical parameters
    ...
    Exp_R_opt = (K- N_optimum)/K * log2(1 + 10^(I_T/10)/(10^(P_tran/10) *  10^(-alpha_true_p/10) +...
            10^(noise_power_p/10)) * nc_th * 10^(-alpha_true_s/10)  / 10^(noise_power_s/10));
    save('results_thr_est_time_tradeoff_AWGN_nu_l_snr_m00_th.mat');
%    quit;
end
...
%print(gcf,'-dpdf','../figures/fig_thr_est_time_tradeoff_AWGN_nu_lower');
%print(gcf,'-depsc','../figures/fig_thr_est_time_tradeoff_AWGN_nu_lower');
