...
hold on,
plot(-alpha_true_p(1:spacing:length(alpha_true_p)) - noise_power,...
    Exp_R_conventional(1:spacing:length(alpha_true_p)), 'k<');
Y_axis_min =  min(Y_axis_min, min(min(Exp_R_opt)));
...
grid on;
axis([-15 10 0.98 8.3]);
ylabel('Throughput at SR [bits/sec/Hz]','FontSize',Fontsize);
xlabel('SNR [dB]','FontSize',Fontsize);
hl = legend([h4 h1 h2 h3],'Ideal Case', '$\pcd = 0.92$', '$\pcd = 0.95$', '$\pcd = 0.97$');
c = get(hl,'Children');
set(c(10),'Marker','<');
set(c(7),'Marker','o');
set(c(4),'Marker','s'); 
...
laprint(1, '../figures/fig_opt_thr_vs_snr_AWGN_ComMag', 'options', 'factory', 'width', 7, 'scalefonts',...
