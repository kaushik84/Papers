...
if simulation == 1
    ...
    lambda_s = 1e-5 * (0.05:0.05:2.5);  % Secondary density   
    ...
    save('results_PPP_CR_TransmitPowerVs_CR_lambda_1e5.mat');
    quit;
end

load('results_PPP_CR_TransmitPowerVs_CR_lambda_1e5.mat')
...
thin_factor_sim = 5;  % Describes the thining in the measurements
...
for a = 1:length(alpha)
    for d = 1:length(d_p)
        ...
        P_s_th_thin = reshape(P_s_th(a,d,:),1,length(lambda_s));
        P_s_th_thin = P_s_th_thin(1:thin_factor_th:length(P_s_th_thin));
    
        lambda_s_th_thin = lambda_s(1:thin_factor_th:length(lambda_s));
...
hl = legend(strcat('th, $\alpha$ = ',num2str(alpha(1))),... 
    strcat('sim, $\alpha$ = ',num2str(alpha(1))),... 
    strcat('th, $\alpha$ = ',num2str(alpha(2))),...
    strcat('sim, $\alpha$ = ',num2str(alpha(2))),...
    strcat('th, $\alpha$ = ',num2str(alpha(3))),...
    strcat('sim, $\alpha$ = ',num2str(alpha(3))),...
    strcat('th, $\alpha$ = ',num2str(alpha(4))),...
...
print(gcf,'-dpdf','../figures/fig_CR_Txp_vs_lambda_CR_1e5');
print(gcf,'-depsc','../figures/fig_CR_Txp_vs_lambda_CR_1e5');
