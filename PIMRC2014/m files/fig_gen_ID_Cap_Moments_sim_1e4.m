...
if simulation == 1
    ...
    save('results_PPP_Moments_Cap_ID_1e4.mat');
    quit;
end
load('results_PPP_Moments_Cap_ID_1e4.mat')
...
for a = 1:length(alpha)
    ...
    elseif a == 2
        ...
        h2 = plot(ax(2), d_s_th, var_Cap_th(a,:),'LineWidth',1.5, 'HandleVisibility','off');
    elseif a == 3
        set(ax,'NextPlot','add');
...
ylabel(ax(2),'$E[C_{ID}^2] - [E[C_{ID}]]^2$','Interpreter','latex','FontSize',Fontsize+2);
...
hl = legend(strcat('th , $\alpha$ = ',num2str(alpha)),...
...
print(gcf,'-dpdf','../figures/fig_ID_Cap_Moments_vs_d_s_1e4');
print(gcf,'-depsc','../figures/fig_ID_Cap_Moments_vs_d_s_1e4');