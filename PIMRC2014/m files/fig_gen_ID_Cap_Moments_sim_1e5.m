...
if simulation == 1
    ...
    save('results_PPP_Moments_Cap_ID_1e5.mat');
    quit;
end
load('results_PPP_Moments_Cap_ID_1e5.mat')
...
for a = 1:length(alpha)
    if a == 1
        [ax h1 h2] = plotyy(d_s_th, Exp_Cap_th(a,:), d_s_th, var_Cap_th(a,:));
        %[ax h1 h2] = plotyy(d_s_th, Exp_Cap_th(a,:), d_s_th, var_Cap_th(a,:));
    ...
    elseif a == 2
        ...
        plot(ax(2), d_s_sim, var_Cap_sim(a,:),'LineStyle','^');
        set(ax,'NextPlot','add');
        h1 = plot(ax(1), d_s_th, Exp_Cap_th(a,:),'LineWidth',1.5, 'HandleVisibility','off');
        h2 = plot(ax(2), d_s_th, var_Cap_th(a,:),'LineWidth',1.5,'LineStyle','--', 'HandleVisibility','off');
    elseif a == 3
        set(ax,'NextPlot','add');
        ...
        set(ax,'NextPlot','add');
        plot(ax(1), d_s_sim, Exp_Cap_sim(a,:),'LineStyle','o');
        plot(ax(2), d_s_sim, var_Cap_sim(a,:),'LineStyle','o');
        set(ax,'NextPlot','add');
        h1 = plot(ax(1), d_s_th, Exp_Cap_th(a,:),'LineWidth',1.5, 'HandleVisibility','off');
        h2 = plot(ax(2), d_s_th, var_Cap_th(a,:),'LineWidth',1.5,'LineStyle','--', 'HandleVisibility','off');
    end
...
hold on;
grid on;
ylabel(ax(1),'E$[C_{ID}]$','Interpreter','latex','FontSize',Fontsize+2);
ylabel(ax(2),'Var$[C_{ID}]$','Interpreter','latex','FontSize',Fontsize+2);
xlabel(ax(1),'$d_s$ = [m]','Interpreter','latex','FontSize',Fontsize+2);
xlabel(ax(2), '$d_s$ = [m]','Interpreter','latex','FontSize',Fontsize+2);
set(ax(1), 'YTick', [0:2:15]);
set(ax(2), 'YTick', [0:2:15]);
hl = legend(strcat('theory'),...
    strcat('simul., $\alpha$ = ',num2str(alpha(1))),...
    strcat('simul., $\alpha$ = ',num2str(alpha(2))),...
    strcat('simul., $\alpha$ = ',num2str(alpha(3))),...
    strcat('simul., $\alpha$ = ',num2str(alpha(4))),...
    strcat('theory'),...
    strcat('simul., $\alpha$ = ',num2str(alpha(1))),...
    strcat('simul., $\alpha$ = ',num2str(alpha(2))),...
    strcat('simul., $\alpha$ = ',num2str(alpha(3))),...
...
axis(ax(1), [min(d_s_sim) max(d_s_sim) 0 15]);
axis(ax(2), [min(d_s_sim) max(d_s_sim) 0 15]);
...
print(gcf,'-dpdf','../figures/fig_ID_Cap_Moments_vs_d_s_1e5');
print(gcf,'-depsc','../figures/fig_ID_Cap_Moments_vs_d_s_1e5');